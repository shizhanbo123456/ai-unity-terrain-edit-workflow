...
| 5 树木编辑 | layerMap + 树池 + 每层密度/scale | 按密度生成位置，**road&lt;0.5 过滤** | 每层 `treePositions`（Vector2[]）+ 密度/scale（层 Config） | **[待开发]**（子界面为占位） |
...
- **⚠️ 透明区域（未绘制位置 = Layer0 / 层ID = -1）需要特殊处理 [待设计]**：不属于任何语义层的空白位置，下游各阶段需统一约定其行为，例如：
  ...
  - 贴图/路网：-1 不在任何组合层内 → 距离场 R 全 0、随机游走不进入、不产生路面。
...
- 烘焙流程：按该层掩码 + 密度生成均匀位置（复用 `UniformPointGenerator`，全局 seed 可复现）→ **过滤 `road >= 0.5` 的位置（不能生成在路上）** → 写入 `layer.treePositions`。
...
- key 约定（共 5 个）：`layerMap / height / distance / occupancy / road` **[已完成]**。树木/细节的位置列表**不存 MapData**（存于各层 Config，见阶段 5/6）。
...
├── LayerConfigSO.cs            [已完成] 每层配置（颜色/名称/权重/高度范围/道路参数；树/细节位置列表字段 **[待开发]**）
...
- **M2 [待开发]** 树木 / 细节子界面（每层密度/scale/位置列表烘焙，road&lt;0.5 过滤）。
